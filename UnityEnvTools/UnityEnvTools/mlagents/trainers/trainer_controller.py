...
class TrainerController(object):
    ...
    def start_learning(self):
        ...
        curr_info = around(self._reset_env())
        if self.train_model:
        ...
        try:
            while any([t.get_step <= t.get_max_steps \
                ...
                new_info = around(self.env.step(vector_action=take_action_vector,
...
def around(info):
    for brain_name, brain_info in info.items():
        info.get(brain_name).vector_observations = np.around(info.get(brain_name).vector_observations, 1)
    return info